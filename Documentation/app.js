...
function renderApiIndex(){
  const groups={};
  visibleTypes().forEach(type=>(groups[type.module]??=[]).push(type));
  apiIndex.innerHTML=Object.entries(groups).map(([module,types])=>`<details open><summary>${escapeHtml(module)} <span>${types.length}</span></summary><div>${types.map(type=>`<button type="button" class="api-type-link${selectedType===type.fullName?' selected':''}" data-type="${escapeHtml(type.fullName)}"><small>${escapeHtml(type.kind)}</small>${escapeHtml(type.name)}</button>`).join('')}</div></details>`).join('')||'<p class="api-empty">Aucun type trouvé.</p>';
